...
- 工具路由
  支持天气查询与联网搜索，并通过意图提取与标准检索链将工具结果结构化注入回复。
- Web 界面
...
├── rag/                           # 标准 RAG 分层
│   ├── tool.py                    # RAGTool 统一接口
│   ├── processing.py              # MarkItDown 转换与 Markdown 智能分块
│   └── models.py                  # 检索计划 / 命中结果模型
│
├── knowledge/
│   ├── persona_system.py          # 人设门面层
│   ├── persona_ingest_service.py  # 人设学习与 RAG 入库
│   ├── persona_preview_service.py # 预览生成与联网补充
│   ├── persona_context_service.py # 人设检索与上下文构建
│   ├── persona_rag_engine.py      # 人设标准 RAG 检索
...
2. `QueryRouter` 决定现实查询还是角色内部问题
3. `RAGTool` 生成检索计划并执行多查询 / HyDE / 混合检索
4. `ToolRouter` 执行天气 / 搜索工具
5. 检索结果与工具结果统一进入最终回复 Prompt
