...
- 先压缩当前会话状态，再选择本轮真正需要的信息
- 角色底色、角色证据、故事证据、用户记忆、工具结果各自分开处理
...
**用户输入 → 意图与任务判断 → 决定是否调用工具 / 召回资料 / 召回记忆 → 情绪与关系评估 → 上下文选择与压缩 → 本轮规划 → Prompt 组装 → LLM 回复 → 状态写回 → 轨迹记录**

更具体一点，一轮回复通常会这样运行：

1. 先判断当前轮更接近设定问答、故事追问、情绪陪伴、现实查询还是普通闲聊  
2. 根据任务决定是否调用工具、是否检索角色资料、是否检索记忆  
3. 根据当前输入和最近互动，评估这轮的情绪氛围和关系影响  
4. 从候选信息里筛选出本轮真正需要的部分，并把旧会话压缩成摘要  
5. 用规划层决定这一轮的回复目标、语气强度、证据边界和保守策略  
6. 组装最终 Prompt，生成角色化回复  
7. 把本轮结果写回记忆、关系、情绪和会话状态，并记录 trace 方便回放和检查
...
graph TD
    ...
    K --> L["LLM 回复生成"]
    L --> M["记忆 / 关系 / 情绪 / 会话状态写回"]
    M --> N["Trace / 持久化 / Replay / Regression"]
```
