  ...
  elements.previewSnippets.innerHTML = snippets.length
    ? snippets.map((item) => `
        <div class="preview-snippet">
          <div class="preview-source">${escapeHtml(item.source)}${item.title ? ` · ${escapeHtml(item.title)}` : ""}</div>
...
function initializeAvatarEditor(image) {
  ...
  state.avatarEditor.minScale = 1;
  state.avatarEditor.maxScale = 4;
...
async function streamAssistantMessage(message) {
  ...
  for (let i = 0; i < bubbles.length; i += 1) {
    ...
    await sleep(i === bubbles.length - 1 ? 80 : 320);
  }
}
...
function renderSnapshot(snapshot, options = {}) {
  ...
  elements.chatTitle.textContent = `${agent.name} 的对话`;
  ...
  elements.agentAvatar.src = agent.avatarUrl || DEFAULT_AGENT_AVATAR;
  ...
  elements.activityList.innerHTML = recentActivity.length
    ? recentActivity.map((item) => `
      ...
      `).join("")
    : `<div class="activity-item"><div class="activity-text">还没有最近活动。</div></div>`;
  if (!preserveHistory) {
...
async function sendMessage() {
  ...
  try {
    ...
    await sleep(180);
    await streamAssistantMessage(assistant);
...
async function uploadPersonaFile() {
  ...
  setTypingStatus("正在整理资料并联网补全…");
  const form = new FormData();
  form.append("file", elements.personaFile.files[0]);
...
async function confirmPreview() {
  ...
  try {
    const data = await requestJson("/api/persona/confirm", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ previewId: state.preview.preview_id }),
    });
    renderSnapshot(data.snapshot);
    renderPreview(null);
    elements.personaText.value = "";
    elements.personaLabel.value = "";
    if (elements.personaFile) {
...
const triggerAutoPreview = () => {
  ...
  autoPreviewTimer = setTimeout(() => {
    previewPersona({ silent: true });
  }, 500);
};
elements.personaText.addEventListener("input", triggerAutoPreview);
elements.personaText.addEventListener("blur", () => previewPersona({ silent: true, text: elements.personaText.value.trim(), label: elements.personaLabel.value.trim() }));
