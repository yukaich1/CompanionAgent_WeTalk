...
from const import AI_SYSTEM_PROMPT, NEW_MEMORY_STATE_PATH, NEW_PERSONA_STATE_PATH
...
from runtime.stage_result import (
    ContextStageResult,
    IntentStageResult,
    MemoryStageResult,
    PersonaRecallStageResult,
    RoutingStageResult,
    ToolStageResult,
)
...
from context.session_context_manager import SessionContextManager
...
class AIConfig(BaseModel):
    name: str = Field(default="WitchTalk")
...
class AISystem:
    def __init__(self, config: AIConfig | None = None):
        ...
        self.turn_traces = deque(maxlen=40)
    ...
    def _memory_note_for_mode(self, response_mode: str, user_input: str, persona_recall=None) -> tuple[str, str]:
        mode = str(response_mode or "casual").strip() or "casual"
        if mode == "self_intro":
            return (
                f"用户请求角色自我介绍：{self._truncate_for_prompt(user_input, 80)}；角色进行了简要身份介绍。",
                "角色进行了简要身份介绍。",
            )
        if mode == "external":
            return (
                f"用户询问现实信息：{self._truncate_for_prompt(user_input, 80)}；角色基于外部证据给出了简洁回答。",
                "角色基于外部证据回答了现实信息。",
            )
        if mode == "story":
            ...
            story_title = ""
            if story_hits:
                story_title = str(story_hits[0].get("title", "") or story_hits[0].get("chunk_id", "") or "").strip()
            title_suffix = f"（{self._truncate_for_prompt(story_title, 40)}）" if story_title else ""
            return (
                f"用户请求角色讲述一个故事；角色讲述了一个命中的故事片段{title_suffix}。",
                "角色讲述了一个命中的故事片段。",
            )
        if mode == "persona_fact":
            return (
                f"用户询问角色设定相关问题：{self._truncate_for_prompt(user_input, 80)}；角色依据已有资料做了回答。",
                "角色依据已有资料回答了设定相关问题。",
            )
        if mode == "emotional":
            return (
                f"用户表达了情绪或需要安抚：{self._truncate_for_prompt(user_input, 80)}；角色先接住情绪并继续对话。",
                "角色先接住了用户情绪并继续对话。",
            )
        if mode == "value":
            return (
                f"用户询问角色看法：{self._truncate_for_prompt(user_input, 80)}；角色表达了自己的态度。",
                "角色表达了自己的态度。",
            )
        return (
            f"用户与角色进行闲聊：{self._truncate_for_prompt(user_input, 80)}；角色自然回应并延续了对话。",
            "角色自然回应了这轮闲聊。",
        )

    def _memory_capture_for_turn(self, user_input: str, response: str, intent_result, persona_recall=None) -> dict[str, str]:
        response_mode = str(getattr(intent_result, "response_mode", "casual") or "casual").strip()
        summary, assistant_note = self._memory_note_for_mode(response_mode, str(user_input or ""), persona_recall=persona_recall)
        user_text = str(user_input or "").strip()
        assistant_text = self._truncate_for_prompt(assistant_note, 120)
        verbatim_excerpt = f"User: {user_text}\nAssistant: {assistant_text}".strip()
        return {
            "summary": self._truncate_for_prompt(summary, 280),
            "user_text": user_text,
            "assistant_text": assistant_text,
            "verbatim_excerpt": self._truncate_for_prompt(verbatim_excerpt, 320),
        }
    ...
    def _extract_intent_stage(self, user_input: str) -> IntentStageResult:
    ...
    def _recall_persona_stage(self, intent_stage: IntentStageResult) -> PersonaRecallStageResult:
        intent_result = intent_stage.intent_result
        ...
        if recall_mode == "none":
            ...
            return PersonaRecallStageResult(
                recall_mode=recall_mode,
                preferred_query_type="external",
        ...
        return PersonaRecallStageResult(
            recall_mode=recall_mode,
            preferred_query_type=preferred_query_type,
            excluded_chunk_ids=exclude_chunk_ids,
            persona_recall=persona_recall,
        )

    def _route_stage(self, intent_stage: IntentStageResult, persona_stage: PersonaRecallStageResult) -> RoutingStageResult:
        route_decision = self.query_router.route(
            intent_stage.normalized_query,
            persona_stage.persona_recall,
            is_public=True,
            recent_conversation=intent_stage.recent_conversation,
            character_name=self.config.name,
            intent_result=intent_stage.intent_result,
        )
        return RoutingStageResult(route_decision=route_decision)

    def _recall_memory_stage(self, normalized_query: str) -> MemoryStageResult:
        working_snapshot = self.memory_system.get_working_memory_snapshot()
        recall_policy = self.memory_system._select_recall_policy(normalized_query, working_snapshot)
        memory_result = self.memory_rag_engine.recall(normalized_query, self.new_memory_state, policy=recall_policy)
        return MemoryStageResult(memory_result=memory_result, recall_policy=recall_policy)

    def _run_tool_stage(self, intent_result, route_decision) -> ToolStageResult:
    ...
    def _assemble_context_stage(
        ...
        *,
        ...
        assembled = self.context_assembler.assemble(
            ...
            deduped=deduped,
            web_persona_context=tool_stage.web_persona_context,
            web_reality_context=tool_stage.web_reality_context,
        )
    ...
    def _recent_dialogue_available(self) -> bool:
        messages = [
            message
            for message in self.get_message_history(False)
    ...
    def _evaluate_evidence_status(
        ...
        if self._memory_sensitive_request(user_input, intent_result):
            return {
                "required": True,
                "ready": memory_ok or self._recent_dialogue_available(),
                "reason": "memory" if (memory_ok or self._recent_dialogue_available()) else "memory_missing",
            }
    ...
    def send_message(self, user_input, return_json: bool = False, attached_image=None):
        response = self.session_runtime.handle_user_turn(str(user_input or ""), attached_image=attached_image)
...
def check_has_valid_key():
    ...
    if not has_llm_api_key():
        print(f"使用 WitchTalk 需要提供 {get_active_llm_label()} API 密钥。")
