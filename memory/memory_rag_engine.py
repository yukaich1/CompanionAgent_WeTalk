...
class MemoryRAGEngine:
    ...
    def recall(self, query: str, state: MemorySystemState, *, policy: str = "episodic_hybrid") -> MemoryRecallResult:
        ...
        stable_records = self._select_stable_records(state, policy=policy)
        filters = infer_recall_filters(query, llm=self.llm)
        try:
            self.rag.embed_chunks(episodic_entries, namespace="memory")
        except Exception:
            pass
        result = self._search_episodic_entries(
            query,
            episodic_entries,
            filters=filters,
            policy=policy,
    ...
    def _search_episodic_entries(self, query: str, episodic_entries: list[dict], *, filters: dict | None, policy: str):
        top_k = 5
        query_type = "memory"
        if policy == "thread_first":
            query_type = "memory"
            top_k = 6
        ...
        return self.rag.search(query, episodic_entries, top_k=top_k, query_type=query_type, namespace="memory")

    def _select_stable_records(self, state: MemorySystemState, *, policy: str) -> list:
        stable_records = list(state.stable_records or [])
        if policy == "thread_first":
            preferred = [record for record in stable_records if str(record.domain or "").strip() in {"ongoing_topic", "interaction_pattern"}]
            return sorted(preferred or stable_records, key=lambda record: record.confidence, reverse=True)[:4]
        if policy == "semantic_first":
            preferred = [
                record
                for record in stable_records
                if str(record.domain or "").strip() in {"user_profile", "user_preference", "relationship"}
            ]
            return sorted(preferred or stable_records, key=lambda record: record.confidence, reverse=True)[:5]
        return sorted(stable_records, key=lambda record: record.confidence, reverse=True)[:3]
