...
class InnateBelief(BaseModel):
    ...
    domain: str = ""
    strength: float = Field(default=0.5, ge=0.0, le=1.0)
    source: KnowledgeSource = KnowledgeSource.USER_CANON
...
class SlowChangeLayer(BaseModel):
    ...
    mutable_habits: list[str] = Field(default_factory=list)
    change_rate_limits: dict[str, float] = Field(
        default_factory=lambda: {"trust": 0.05, "affection": 0.03, "respect": 0.04}
    )
...
# ---- Runtime implementation merged from persona_runtime.py ----
...
from persona_models import PersonaKeywordOption, PersonaPreviewModel, PersonaSourceSnippet, PersonaStatusModel, PersonaSummaryModel
...
SECTION_KEYWORDS = {
    ...
    "catchphrases": ["口头禅", "常说", "惯用语", "经典台词", "标志性说法", "反复提及"],
    "addressing_habits": ["称呼", "自称", "叫法", "如何称呼", "怎么称呼别人", "怎么称呼自己"],
    ...
    "life_experiences": ["经历", "故事", "曾经", "遇到", "旅行", "过去"],
}
...
class PersonaSystem:
    ...
    def _repair_state(self):
        ...
        for field in ("dynamic_meta_exclusions", "dynamic_novel_hints", "dynamic_voice_hints", "dynamic_trait_markers", "display_keywords", "selected_keywords", "style_examples", "natural_reference_triggers"):
    ...
    def _is_meta_commentary(self, text):
        ...
        if any(word in text for word in AUDIENCE_PERSPECTIVE_WORDS) and any(word in text for word in META_REACTION_WORDS):
            return True
        if any(word in text for word in ("吸引", "关于", "对于", "评价")) and any(word in text for word in AUDIENCE_PERSPECTIVE_WORDS):
            return True
        return False
    ...
    def _is_display_keyword_candidate(self, token):
        ...
        if token in {
            ...
            return False
        if any(mark in token for mark in ("的", "人", "是", "在", "和", "中", "又", "被", "把", "让", "就", "很", "可", "并")):
            return False
    ...
    def _fact_to_keywords(self, value):
        ...
        for piece in pieces:
            for token in DYNAMIC_KEYWORD_RE.findall(piece):
                if token in {"这个", "那个", "一种", "一人", "自己", "别人", "因为", "所以", "如果", "但是", "然后", "感觉", "有点", "其实", "真的", "非常", "比较", "不是", "可以"}:
                    continue
                result.append(token)
    ...
    def _extract_profile_facts(self, raw_text):
        ...
        for line in re.split(r"\n+", raw_text or ""):
            ...
            if any(marker in stripped for marker in TRAIT_MARKERS) or "“" in stripped or "”" in stripped:
                parts = [
                    part.strip(" :：；，,。")
                    for part in TRAIT_SPLIT_RE.split(stripped)
                    if part.strip(" :：；，,。")
                ]
                for part in parts:
        ...
        for sentence in self._split_sentences(raw_text):
            ...
            for quote in re.findall(r"[“\"]([^”\"]{3,60})[”\"]", sentence):
                ...
                if quote.endswith(("呀", "啦", "呢", "哦", "嘛", "哼", "啊", "呐")):
    ...
    def _merge_summary_data(self, summary):
        ...
        self.style_examples = _dedupe(self.style_examples + summary.get("style_examples", []), limit=8)
        self.natural_reference_triggers = _dedupe(
            self.natural_reference_triggers + summary.get("natural_reference_triggers", []),
            limit=10,
        )

    def _selected_keyword_candidates(self, summary):
        summary = self._summary_to_dict(summary)
        candidates = []
        candidates.extend(summary.get("display_keywords", []))
        for section in ("personality", "values", "worldview", "core_setup"):
            candidates.extend(summary.get("section_keywords", {}).get(section, []))
        filtered = []
        ...
        return _dedupe(filtered, limit=8)

    def _persona_priority_lines(self):
        lines = []
        selected_keywords = _dedupe(self.selected_keywords, limit=8)
        if selected_keywords:
            lines.append("[User-Selected Persona Priorities]")
            lines.extend(f"- {value}" for value in selected_keywords)
            lines.append("- 这些关键词只是高权重方向，不要机械复读。")
        if self.profile.get("voice_style") or self.style_examples:
            lines.append("[Priority Voice and Delivery]")
            for value in self.profile.get("voice_style", [])[:5]:
                lines.append(f"- {value}")
            for value in self.style_examples[:4]:
                lines.append(f"- Example: {value}")
        if self.profile.get("addressing_habits") or self.profile.get("sentence_endings"):
            lines.append("[Priority Conversational Habits]")
            lines.extend(f"- {value}" for value in self.profile.get("addressing_habits", [])[:4])
            lines.extend(f"- {value}" for value in self.profile.get("sentence_endings", [])[:4])
        for section in ("catchphrases", "addressing_habits", "sentence_endings", "likes", "dislikes", "life_experiences"):
            values = self.profile.get(section, [])
            if values:
                lines.append(f"[Priority {SECTION_TITLES[section]}]")
                lines.extend(f"- {value}" for value in values[:4])
        if self.natural_reference_triggers:
            lines.append("[Natural Mention Triggers]")
            lines.extend(f"- {value}" for value in self.natural_reference_triggers[:6])
        return lines

    def _detect_relevant_sections(self, query):
        query = self._normalize_fact(query, max_chars=160)
        relevant = []
        if not query:
            return relevant
        for section in SECTION_ORDER:
            if any(keyword in query for keyword in self._all_section_keywords(section)):
                relevant.append(section)
        if any(token in query for token in ("故事", "经历", "过去", "旅行", "背景", "原作", "剧情")):
            relevant.extend(["life_experiences", "core_setup"])
        if any(token in query for token in ("说话", "语气", "口头禅", "称呼", "句尾", "口癖", "自我介绍")):
            relevant.extend(["voice_style", "catchphrases", "addressing_habits", "sentence_endings", "core_setup"])
        if any(token in query for token in ("喜欢", "讨厌", "厌恶", "喜好")):
            relevant.extend(["likes", "dislikes"])
        if any(token in query for token in ("性格", "特点", "特征")):
            relevant.append("personality")
        if any(token in query for token in ("价值观", "世界观", "原则", "信念", "想法", "看法")):
            relevant.extend(["values", "worldview"])
        return _dedupe(relevant)

    def build_precise_query_context(self, query, top_k=6, char_budget=900):
        if not query:
            return ""
        relevant_sections = self._detect_relevant_sections(query)
        local_lines = []
        seen = set()
        for section in relevant_sections:
            values = self.profile.get(section, [])
            if values:
                local_lines.append(f"[Precise {SECTION_TITLES[section]}]")
                local_lines.extend(f"- {value}" for value in values[:5])
        search_pool = self._keyword_search_entries(query, top_k=top_k * 3) + self._search_entries(query, top_k=top_k * 3)
        scored = []
        ...
        for entry in search_pool:
            ...
            seen.add(text)
            score = 0.0
            if entry.get("kind", "") == "core_summary":
                score += 2.0
            score += 0.35 * sum(1 for term in query_terms if term in text)
            for section in relevant_sections:
                if any(keyword in text for keyword in self._all_section_keywords(section)):
                    score += 1.6
        ...
        blocks = []
        if local_lines:
            blocks.append("Precise Persona Facts:\n" + "\n".join(local_lines))
        if evidence_lines:
            blocks.append("Precise Persona Evidence:\n" + "\n".join(evidence_lines))
        context = "\n\n".join(blocks).strip()
        if len(context) > char_budget:
            context = context[: char_budget - 3].rstrip() + "..."
        return context

    def _build_keyword_options(self, snippets, source_label):
        options = []
        for index, snippet in enumerate(snippets):
            snippet_summary = self._summarize_with_llm(
                snippet.text,
                f"{source_label}#{snippet.source}_{index}_{snippet.title or 'snippet'}",
            ) or PersonaSummaryModel()
            keywords = self._selected_keyword_candidates(snippet_summary)
            if not keywords:
                continue
            options.append(
                PersonaKeywordOption(
    ...
    def _commit_summary_and_entries(self, raw_text, source_label, summary, progress_callback=None):
        ...
        if not pending_entries:
            ...
            self._save_to_disk()
            self._emit_progress(progress_callback, 100, 100, "done", "娌℃湁鏂板鍐呭锛屽凡瀹屾垚")
            return 0
    ...
    def load_text(self, raw_text, source_label="manual_input", progress_callback=None):
        ...
        self._emit_progress(progress_callback, 12, 100, "summary", "姝ｅ湪鎻愬彇鏍稿績浜鸿")
        summary = self._summarize_with_llm(raw_text, source_label)
        self._emit_progress(progress_callback, 28, 100, "chunking", "姝ｅ湪鍒囧垎瀛︿範鏂囨湰")
    ...
    def load_file(self, filepath, progress_callback=None):
        ...
        self._emit_progress(progress_callback, 0, 100, "reading", f"姝ｅ湪璇诲彇 {os.path.basename(filepath)}")
    ...
    def _build_persona_search_queries(self, persona_name, work_title=""):
        anchor = " ".join(part for part in [work_title, persona_name] if part).strip() or persona_name.strip()
        queries = [
            " ".join(part for part in [anchor, "角色设定", "性格", "说话方式", "口头禅", "价值观", "经历"] if part),
            " ".join(part for part in [anchor, "角色介绍", "人物设定"] if part),
            " ".join(part for part in [anchor, "经典台词", "人物经历"] if part),
            anchor,
        ]
        return _dedupe([query for query in queries if query], limit=4)

    def _is_relevant_web_snippet(self, item, persona_name, work_title=""):
        combined = self._normalize_search_text(f"{item.get('title', '')} {item.get('text', '')}")
        persona_key = self._normalize_search_text(persona_name)
        work_key = self._normalize_search_text(work_title)
        if persona_key and persona_key not in combined:
            return False
        if work_key and work_key not in combined:
            if not any(marker in combined for marker in ("角色", "人物", "设定", "经历", "台词", "性格")):
                return False
        return True

    def _collect_web_snippets(self, persona_name, work_title="", local_canon="", max_results=5):
        query_candidates = self._build_persona_search_queries(persona_name, work_title)
        snippets = []
        seen_keys = set()
        seen_texts = set()
        for query in query_candidates:
            ...
            for item in search_result.get("snippets", []):
                ...
                if not text:
                    continue
                if not self._is_relevant_web_snippet(item, persona_name, work_title):
                    continue
                decision = self.conflict_filter.should_keep(
                    text=text,
                ...
                if not decision.kept:
                    continue
                canonical_text = self._canonicalize_source_text(text)
    ...
    def _summarize_web_snippets(self, persona_name, work_title, snippets):
        if not snippets:
            return ""
        reference_text = "\n\n".join(
            f"[{snippet.source} | {snippet.title}]\n{snippet.text}" for snippet in snippets[:6]
        )[:4800]
        prompt = (
            "请根据以下公开资料，只总结与当前角色本人强相关的人设补充信息。\n"
            "重点只允许保留：人物特点、说话方式、价值观、经历过的关键故事。\n"
        ...
        try:
            summary = self.model.generate(prompt, temperature=0.1, max_tokens=600)
        except Exception:
            return ""
        return str(summary or "").strip()

    def preview_from_sources(self, persona_name="", work_title="", local_text="", local_label="manual_input", max_results=5, local_snippets=None):
        ...
        if persona_name:
            web_snippets = self._collect_web_snippets(
                persona_name,
                work_title=work_title,
                local_canon=local_canon,
                max_results=max_results,
            )
            web_summary = self._summarize_web_snippets(persona_name, work_title, web_snippets)
            if web_summary:
                snippets.append(
                    PersonaSourceSnippet(
                        source="web_summary",
        ...
        keyword_options = self._build_keyword_options(snippets, source_label)
        selected_keywords = _dedupe(
            [keyword for option in keyword_options for keyword in option.keywords] + summary.display_keywords,
            limit=8,
        )
    ...
    def _commit_summary_only(self, source_label, summary, selected_keywords=None):
        if not summary:
            return
        self._merge_summary_data(summary)
        if selected_keywords is not None:
            self.selected_keywords = _dedupe(selected_keywords, limit=8)
        self._append_core_summary(summary, source_label)
        self._dedupe_storage()
        self._save_to_disk()
    ...
    def confirm_preview(self, preview_id, selected_keywords=None, progress_callback=None):
        ...
        chosen_keywords = _dedupe(
            [str(item).strip() for item in (selected_keywords if selected_keywords is not None else preview.selected_keywords)],
            limit=8,
        )
        if not chosen_keywords:
            chosen_keywords = _dedupe(preview.summary.display_keywords, limit=8)
        count = 0
        ...
        self._commit_summary_only(preview.source_label, preview.summary, chosen_keywords)
    ...
    def _extract_query_terms(self, query):
        ...
        for token in DYNAMIC_KEYWORD_RE.findall(text):
            if len(token) < 2:
                continue
            if token in {"角色", "设定", "故事", "经历", "背景", "一个", "什么", "怎么", "这个", "那个", "有关", "关于"}:
                continue
            terms.append(token)
    ...
    def get_display_keywords(self, limit=DISPLAY_KEYWORD_LIMIT):
        candidates = []
        candidates.extend(self.selected_keywords)
    ...
    def build_context(self, query):
        ...
        canon_lines = []
        voice_focus = []
        precise_context = self.build_precise_query_context(query, top_k=5, char_budget=700)
        if self.profile.get("voice_style"):
            voice_focus.append("[Voice Style]")
            voice_focus.extend(f"- {value}" for value in self.profile.get("voice_style", [])[:6])
        if self.style_examples:
            voice_focus.append("[Voice Examples]")
            voice_focus.extend(f"- {value}" for value in self.style_examples[:5])
        if self.profile.get("catchphrases"):
            voice_focus.append("[Catchphrases]")
            voice_focus.extend(f"- {value}" for value in self.profile.get("catchphrases", [])[:4])
        if self.profile.get("addressing_habits"):
            voice_focus.append("[Addressing Habits]")
            voice_focus.extend(f"- {value}" for value in self.profile.get("addressing_habits", [])[:4])
        if self.profile.get("sentence_endings"):
            voice_focus.append("[Sentence Endings]")
            voice_focus.extend(f"- {value}" for value in self.profile.get("sentence_endings", [])[:4])
        if voice_focus:
            sections.append("Priority Roleplay Voice:\n" + "\n".join(voice_focus))
        if precise_context:
            sections.append(precise_context)
        for section in SECTION_ORDER:
            values = self.profile.get(section, [])
            if values:
                canon_lines.append(f"[{SECTION_TITLES[section]}]")
                canon_lines.extend(f"- {value}" for value in values[:6])
        canon_lines.extend(self._persona_priority_lines())
