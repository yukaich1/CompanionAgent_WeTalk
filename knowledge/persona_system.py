...
class PersonaSystem:
    def __init__(self, persona_name: str = "WitchTalk"):
        self.persona_name = persona_name or "WitchTalk"
    ...
    def __setstate__(self, state):
        ...
        self.__dict__.pop("_serialized_index", None)
        self.__dict__.pop("index", None)
        self.__dict__.pop("index_dim", None)
        self.model = FallbackMistralLLM()
        self.dimension_titles = DIMENSION_TITLES_ZH
        self.dynamic_keyword_re = DYNAMIC_KEYWORD_RE
        self.normalize_vector = normalize_vector
    ...
    def _repair_state(self):
        ...
        if self.entries:
            self.ingest_service.sync_vector_store_from_entries()
    ...
    def clear(self):
        ...
        self.style_examples = []
        self.character_voice_card = ""
        self.ingest_service.rag.clear_namespace("persona")
    ...
    def _dedupe_storage(self):
        ...
        for entry in self.entries:
            ...
            if kind not in {"source_chunk", "story_chunk"}:
                continue
            if not text or (kind, text) in seen or (chunk_id and chunk_id in seen_chunk_ids):
                continue
            seen.add((kind, text))
            if chunk_id:
