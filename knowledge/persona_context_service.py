...
class PersonaContextService:
    ...
    def build_precise_query_context(self, query: str, top_k: int = 5, char_budget: int = 700) -> str:
        ...
        entries = [entry for entry in list(self.system.entries or []) if str(entry.get("kind") or entry.get("metadata", {}).get("kind") or "") in kinds]
        result = self.rag.search(query, entries, top_k=top_k, query_type=plan.query_type or "persona")
